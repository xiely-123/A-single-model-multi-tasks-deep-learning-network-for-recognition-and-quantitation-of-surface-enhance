...
print(device)
net=net.double()  
criterion_re = torch.nn.MSELoss() # #####CE准则loss torch.nn.CrossEntropyLoss()
...
Train_list = np.loadtxt("./train_real_3.txt", dtype=str, delimiter=',')
devel_list = np.loadtxt("./test_real_3.txt", dtype=str, delimiter=',')
...
for epoch in range(start_epoch,num_epochs):
    ...
    for i in range(int(len(Train_list)/batch_size)+1):
        ...
        print('\r','Epoch[{}/{}],Process[{}/{}],loss:{:.6f},ACC5:{:.6f},ACC2:{:.6f},time:{:.3f},leanring_rate:{:.6f}'.format(epoch + 1, num_epochs, i + 1, int(len(Train_list)/batch_size)+1, loss.item(), running_corrects1/len(label),running_corrects1/len(label_1), train_time, param_group['lr']),end='')
    running_corrects = torch.zeros(1).squeeze().cuda()
